...
// 1) B+. Repite código por no usar herencia correctamente.
// 2) MB.
// 3) MB-.
// 4) MB.
// 5) MB.
...
class Prenda {
	...
	method incrementarDesgaste(valor) {
		desgaste += valor
	}
...
class PrendaParIzq inherits Prenda {
	// Por qué esto no está en la superclase
...
class Hijo {
	...
	method talleMio() = talle.talle() // Innecesario
...
class Familia {
	...
	method hijosChiquitos() = ninios.filter({ ninio => ninio.edad() < 4 }) // Falta abstracción esPequenio
